...
class RealNVP(nn.Module):
    ...
    def g(self, z1, z2, z3):
        ...
        x1, x_off_1 = self.factor_out(z1, self.order_matrix_1)
        x2, x_off_1 = self.factor_out(z2, self.order_matrix_1)
        x3, x_off_1 = self.factor_out(z3, self.order_matrix_1)


        if self.datainfo.name in ['imnet32', 'imnet64', 'celeba']:
            x1, x_off_2 = self.factor_out(x1, self.order_matrix_2)
            x1, x_off_3 = self.factor_out(x1, self.order_matrix_3)

            x2, x_off_2 = self.factor_out(x2, self.order_matrix_2)
            x2, x_off_3 = self.factor_out(x2, self.order_matrix_3)
            ...
            if(self.datainfo.name == "imnet32" or self.datainfo.name == "celeba"):
                ...
                x1 = self.restore(x1, x_off_3, self.order_matrix_3)
                x2 = self.restore(x2, x_off_3, self.order_matrix_3)
                x3 = self.restore(x3, x_off_3, self.order_matrix_3)
    ...
    def f(self, x):
        ...
        if self.datainfo.name in ['imnet32', 'imnet64', 'celeba']:
            ...
            z1 = self.restore(z1, z_off_3, self.order_matrix_3)
            log_diag_J1 = self.restore(log_diag_J1, log_diag_J_off_3, self.order_matrix_3)

            z2 = self.restore(z2, z_off_3, self.order_matrix_3)
            log_diag_J2 = self.restore(log_diag_J2, log_diag_J_off_3, self.order_matrix_3)

            z3 = self.restore(z3, z_off_3, self.order_matrix_3)
            log_diag_J3 = self.restore(log_diag_J3, log_diag_J_off_3, self.order_matrix_3)

            z1 = self.restore(z1, z_off_2, self.order_matrix_2)
            log_diag_J1 = self.restore(log_diag_J1, log_diag_J_off_2, self.order_matrix_2)

            z2 = self.restore(z2, z_off_2, self.order_matrix_2)
            log_diag_J2 = self.restore(log_diag_J2, log_diag_J_off_2, self.order_matrix_2)

            z3 = self.restore(z3, z_off_2, self.order_matrix_2)
            log_diag_J3 = self.restore(log_diag_J3, log_diag_J_off_2, self.order_matrix_2)

            
        
        z1 = self.restore(z1, z_off_1, self.order_matrix_1)
        log_diag_J1 = self.restore(log_diag_J1, log_diag_J_off_1, self.order_matrix_1)

        z2 = self.restore(z2, z_off_1, self.order_matrix_1)
        log_diag_J2 = self.restore(log_diag_J2, log_diag_J_off_1, self.order_matrix_1)

        z3 = self.restore(z3, z_off_1, self.order_matrix_1)
        log_diag_J3 = self.restore(log_diag_J3, log_diag_J_off_1, self.order_matrix_1)

        return [z1,z2,z3], [log_diag_J1,log_diag_J2,log_diag_J3]
    ...
    def log_prob(self, x):
        ...
        log_diag_J1, log_diag_J2, log_diag_J3 = log_diag_J[0], log_diag_J[1], log_diag_J[2]

        log_det_J1 = torch.sum(log_diag_J1, dim=(1, 2, 3))
        log_det_J2 = torch.sum(log_diag_J2, dim=(1, 2, 3))
        log_det_J3 = torch.sum(log_diag_J3, dim=(1, 2, 3))

        log_prior_prob1 = torch.sum(self.prior.log_prob(z1), dim=(1, 2, 3))
        log_prior_prob2 = torch.sum(self.prior.log_prob(z2), dim=(1, 2, 3))
        log_prior_prob3 = torch.sum(self.prior.log_prob(z3), dim=(1, 2, 3))

        return [log_prior_prob1 + log_det_J1, log_prior_prob2 + log_det_J2, log_prior_prob3 + log_det_J3]
    ...
    def sample(self, size):
        ...
        z1 = self.prior.sample((size, C, H, W))
        z2 = self.prior.sample((size, C, H, W))
        z3 = self.prior.sample((size, C, H, W))
        return self.g(z1, z2, z3)
